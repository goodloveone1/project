  ...
  <table class="table" id="Datatable">
    ...
    <tbody>
			<?php
						...
						$asm= mysqli_query($con,"SELECT ass_id,year_id,st.st_id,st.fname,st.lname,st.position FROM assessments AS ass 
						INNER JOIN staffs AS st ON ass.staff = st.st_id 
						WHERE ass.staff != '$_SESSION[user_id]' AND st.branch_id='$_SESSION[branch]' AND st.permiss_id = '2' AND year_id='$_POST[year]' AND ass_id LIKE'TOR%' ORDER BY year_id DESC") or  die("SQL Error==> ".mysqli_error($con));
    			...
    			echo "<td>$pos_name</td>";
					...
					if(empty($evd_status)){
							...
							echo "<td class='text-center'> <b class='btn text-info'><i class='far fa-clock fa-2x'></i><br>รอบุคลากรอัปโหลดหลักฐาน </br></td>";
					}else if($evd_status == 1){
						echo "<td class='text-center'><b class='text-danger'><i class='far fa-clock fa-2x'></i><br>รอยืนยันอีกครั้ง </b></td>"; 
							echo "  <td class='text-center'> <b class='btn text-info' ><i class='far fa-clock fa-2x'></i><br>รอบุคลากรยืนยันหลักฐาน </b></td>";
